...
class _UserProfileScreenState extends ConsumerState<UserProfileScreen> {
  ...
  Widget _buildManagedEventsSection() {
    if (!_userData!.showManagedEvents) {
      return const SizedBox.shrink();
    }

    // 運営イベントの取得にはFirebase UIDを使用する（イベントデータがFirebase UIDで保存されているため）
    final managedEventsAsync = ref.watch(
      publicManagedEventsProvider((
        userId: _userData!.id, // カスタムIDではなくFirebase UIDを使用
        showManagedEvents: _userData!.showManagedEvents,
      )),
    );
    ...
    return managedEventsAsync.when(
      data: (events) {
        ...
        final hasMoreEvents = events.length > maxDisplayCount;
  ...
  Widget _buildParticipatingEventsSection() {
    ...
    final participatingEventsAsync = ref.watch(
      publicParticipatingEventsProvider((
